...
const blankItem = () => ({ product: '', grade: '', qtyKg: '', priceKg: '' });
const blank = () => ({
  orderNo: 'DH' + today().replace(/-/g, '').slice(2) + '-' + Math.random().toString(36).slice(2, 5).toUpperCase(),
  date: today(), customerId: '', customerName: '', items: [blankItem()], vatPct: 0, status: 'Đã chốt',
  deliveryDate: '', paymentDays: 0, dueDate: '', note: '', custom: {},
});
...
export default function Orders() {
  ...
  const sum = useMemo(() => {
    ...
    return {
      ...
      quotes: rows.filter((r) => r.status === 'Báo giá').length,
    };
  }, [rows]);
  ...
  const doExport = () => {
    ...
    rows.forEach((r) => (r.items || []).forEach((it) => lines.push({
      'Số ĐH': r.orderNo, Ngày: fmtDate(r.date), 'Nhân viên': staffName(r.ownerEmail), 'Khách hàng': r.customerName,
      'Sản phẩm': it.product, 'Mã/Grade': it.grade, 'SL (kg)': num(it.qtyKg), 'Đơn giá (đ/kg)': num(it.priceKg),
      'Thành tiền': num(it.qtyKg) * num(it.priceKg), 'VAT %': num(r.vatPct), 'Trạng thái': r.status,
    })));
    exportSheets(`DonHang_${range.from}_${range.to}`, {
      'Đơn hàng': rows.map((r) => ({
        'Số ĐH': r.orderNo, Ngày: fmtDate(r.date), 'Nhân viên': staffName(r.ownerEmail), 'Khách hàng': r.customerName,
        'Sản phẩm': (r.items || []).map((i) => `${i.product} ${i.grade || ''}`.trim()).join('; '),
    ...
    <>
      <div className="page-head">
        ...
        <div className="actions">
          ...
          <button className="btn primary" onClick={() => setEdit(blank())}>+ Tạo đơn / báo giá</button>
        </div>
      </div>
      ...
      <div className="stats">
        <Stat label="Đơn đã chốt" value={sum.count} sub={`${sum.quotes} báo giá đang chờ`} />
        ...
        {rows.length === 0 ? <Empty /> : (
          <table>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  <td className="nowrap">{r.orderNo}</td>
                  <td className="nowrap">{fmtDate(r.date)}</td>
                  {isAdmin && <td>{staffName(r.ownerEmail)}</td>}
                  <td>{r.customerName}</td>
                  <td>{(r.items || []).map((i, k) => <div key={k}>{i.product} {i.grade} <span className="small">{fmtNum(i.qtyKg, 0)}kg × {fmtMoney(i.priceKg)}</span></div>)}</td>
      ...
      {edit && <OrderForm initial={edit} onClose={() => setEdit(null)} profile={profile} config={config} fields={fields} />}
    </>
  );
}

function OrderForm({ initial, onClose, profile, config, fields }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const items = rest.items.filter((it) => it.product || num(it.qtyKg)).map((it) => ({ ...it, qtyKg: num(it.qtyKg), priceKg: num(it.priceKg) }));
      const dueDate = num(rest.paymentDays) > 0 ? addDays(rest.deliveryDate || rest.date, num(rest.paymentDays)) : (rest.dueDate || rest.date);
      await saveDoc('orders', id, {
  ...
  return (
    <Modal title={f.id ? 'Sửa đơn hàng' : 'Tạo đơn hàng / báo giá'} onClose={onClose} wide>
      <form onSubmit={submit}>
        ...
        <div className="table-wrap items-table">
          <table>
            <thead><tr><th>Loại hạt</th><th>Mã / Grade</th><th className="num">SL (kg)</th><th className="num">Đơn giá (đ/kg)</th><th className="num">Thành tiền</th><th></th></tr></thead>
            <tbody>
              {f.items.map((it, i) => (
                <tr key={i}>
                  <td>
                    <input list="product-list" value={it.product} onChange={(e) => setItem(i, 'product', e.target.value)} placeholder="PP, PE…" />
                  </td>
                  <td><input value={it.grade} onChange={(e) => setItem(i, 'grade', e.target.value)} placeholder="VD: T30S" /></td>
                  <td><input type="number" step="any" value={it.qtyKg} onChange={(e) => setItem(i, 'qtyKg', e.target.value)} /></td>
                  <td><input type="number" step="any" value={it.priceKg} onChange={(e) => setItem(i, 'priceKg', e.target.value)} /></td>
                  <td className="num">{fmtMoney(num(it.qtyKg) * num(it.priceKg))}</td>
                  <td>{f.items.length > 1 && <button type="button" className="btn sm danger" onClick={() => setF({ ...f, items: f.items.filter((_, j) => j !== i) })}>✕</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <button type="button" className="btn sm" style={{ marginTop: 8 }} onClick={() => setF({ ...f, items: [...f.items, blankItem()] })}>+ Thêm dòng</button>
        <div className="form-grid" style={{ marginTop: 12 }}>
          <Field label="VAT % (để 0 nếu giá đã gồm VAT)">
            <input type="number" step="any" value={f.vatPct} onChange={set('vatPct')} />
          </Field>
